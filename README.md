...
| 08 | 控制舵机动作和运动时间 | IDDATA填1-16，角度DATA填0-180，时间DATA填1-250，MAX 4*3=12字节 ，4个舵机数据（ID+角度+时间） 总帧长20字节|
...
STM32 -> 上位机
数据长度统一20字节，没用到字节发0，最后一字节为0xFE，倒数一字节为校验和。
...
| 帧头(1B) | 协议版本 (1B) | 指令 (1B) | 数据(1B) | 数据(1B) | 数据(1B) | 数据(nB) |校验 (1B) | 帧尾 (1B)
|--|--|--|--|--|--|--|--|--|
| 0xFF | 0x01 | 0x08 | ID | 角度 | 时间 | 电机数据n | CHECKSUM | 0xFE |
...
ID+角度+运行时间 各1字节，一帧最大电机数据为4组，帧头+协议+指令+4*3字节+电机动作个数+校验+帧尾 一帧20字节
