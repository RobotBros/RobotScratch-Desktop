...
| 08 | 控制舵机动作和运动时间 | IDDATA填1-16，角度DATA填0-180，时间DATA填1-250，MAX 5*3=15字节  5个舵机数据（ID+角度+时间） 总帧长20字节|
...
ID+角度+运行时间 各1字节，一帧最大电机数据为40组，帧头+协议+指令+5*3字节+电机动作个数+校验+帧尾 一帧20字节
